...
def tokenize_and_save(entity: str, layer, out_path: str, context_length=32):
    ...
    for i, (k, v) in tqdm(enumerate(layer.items())):
        if entity == "title":
            ...
            tokenized_texts = []
            ingrs = [' '.join(ing["text"].lower().split(' ')[:context_length]) for ing in v["ingredients"]]
            for text in ingrs:
                tokenized_text = clip.tokenize(text,context_length=context_length*20)
                tokenized_text = tokenized_text[tokenized_text != 0]
            ...
            tokenized_texts = []
            ingrs = [' '.join(ing["text"].lower().split(' ')[:context_length]) for ing in v["instructions"]]
            for text in ingrs:
                tokenized_text = clip.tokenize(text,context_length=context_length*20)
                tokenized_text = tokenized_text[tokenized_text != 0]
                tokenized_texts.append(tokenized_text)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_layer", type=str, default='/srv/datasets/FoodLog/recipe1M/layer/layer1.json', help="path to layer 1")
    parser.add_argument("--output_path_layer1",type=str,default='/srv/datasets2/recipe1m+/recipe1m_3.10/clip_text/tokenized_recipe.json')
    parser.add_argument("--output_path_tokenized_texts", type=str, default='/srv/datasets2/recipe1m+/recipe1m_3.10/clip_text')
