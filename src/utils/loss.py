...
class DynamicTripletLoss(nn.Module):
    ...
    Args:
        margin (float): loss margin
    """
    def __init__(self, margin=0.1, reduction=None):
    ...
    def loss(self, im: torch.Tensor, rec: torch.Tensor, margin: Optional[torch.Tensor] = None) -> torch.Tensor:
        i2r_scores = self.distance_function(F.normalize(im, dim=-1),F.normalize(rec, dim=-1))
        pos = torch.eye(im.size(0),dtype=torch.bool).to(im.device)
        # neg = torch.logical_not(pos).to(im.device)
...
class DynamicWeightedTripletLoss(DynamicTripletLoss):
    def __init__(self, margin=0.1, reduction=None):
        super(DynamicWeightedTripletLoss, self).__init__(margin, reduction)

    def cal_weight(self, nutrs:torch.Tensor, num_nutrs:int = 4, scale=0.05, **kwargs) -> torch.Tensor:
        sum_weight = torch.zeros((len(nutrs), len(nutrs)), dtype=torch.float, device=nutrs.device)
        for i in range(num_nutrs):
            nutr_combs = torch.combinations(nutrs[:,i],with_replacement=True)
            errs = torch.abs(nutr_combs[:,0] - nutr_combs[:,1])
...
class DynamicWeightedMultiTripletLoss(DynamicWeightedTripletLoss):
    def __init__(self, margin=0.1, reduction=None):
        super(DynamicWeightedMultiTripletLoss, self).__init__(margin, reduction)
    def forward(self, target_dict: Dict[str, torch.Tensor], nutrs: torch.Tensor, **kwargs) -> torch.Tensor:
        weight = self.cal_weight(nutrs)
        keys = list(target_dict.keys())
        losses = torch.tensor(0.,device=target_dict[keys[0]].device)
        ...
        for src_key in keys:
            ...
            if not len(keys):
                break
            for tgt_key in keys:
                loss = self.loss(target_dict[src_key],target_dict[tgt_key],margin=weight)
                losses += loss
                count += 1
        return losses / count
...
def get_loss(config):
    ...
    elif loss_name == 'dynamic_triplet':
        return DynamicWeightedTripletLoss(), DynamicWeightedMultiTripletLoss()
    else:
