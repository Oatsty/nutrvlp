...
def train_one_epoch(config, dataloaders: Dict[str, DataLoader], recipe_model: nn.Module, img_model: nn.Module, im2rec_loss_function: nn.Module, recipe_loss_function: nn.Module, optim, device):
    ...
    for phase in ['train','val']:
        ...
        for batch in tqdm(dataloaders[phase]):
            ...
            out_recipe, out_comp_embs = recipe_model(recipe_input,nutr)
            out_img = img_model(img_input)
            loss = im2rec_loss_function(out_img,out_recipe,nutrs=nutr) + recipe_loss_weight * recipe_loss_function(out_comp_embs,nutrs=nutr)
            if phase == 'train':
                loss.backward()
                optim.step()
...
def train(config, dataloaders: Dict[str, DataLoader], recipe_model: nn.Module, img_model: nn.Module, im2rec_loss_function: nn.Module, recipe_loss_function: nn.Module, optim, scheduler, device):
    ...
    for epoch in range(num_epochs):
        ...
        avg_loss_dict = train_one_epoch(config, dataloaders, recipe_model, img_model, im2rec_loss_function, recipe_loss_function, optim, device)
...
def main():
    ...
    loss_function, recipe_loss_function = get_loss(config)
    ...
    train(config, dataloaders, recipe_model, img_model, loss_function, recipe_loss_function, optim, scheduler, device)
