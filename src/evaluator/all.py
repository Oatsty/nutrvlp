...
from dataset import create_data, create_recipe_retrieval_data
from model.tfood import create_tfood_model, create_tfood_clip_model
# from model.nutr_car import create_nutr_car_model, create_nutr_car_model_old
from model.nutr_ht import create_nutr_ht_model
from model.nutr_tfood import create_nutr_tfood_model
...
from utils.misc import convert_state_dict, convert_tfood_clip_state_dict
...
class VLPEvaluator(object):
    def __init__(self, config, device) -> None:
        ...
        self.dataloader = create_data(config, 'test')
        logger.info('creating models')
        self.model = create_vlp_model(config,device)
    ...
    def evaluate(self):
        ...
        for batch in tqdm(self.dataloader):
            ...
            img_input = batch['image']['data'].to(self.device)
            nutr = batch['recipe']['nutr'].to(self.device)
            out = self.model(recipe_input,img_input)
            out_recipe = out['recipe_embedding']
            out_img = out['image_embedding']
            loss = self.loss_function(out_img,out_recipe)
            running_loss += loss.item()
            instance_count += len(out_img)
            evaluator_recipe(out_img, out_recipe, batch)
        avg_loss = running_loss / instance_count
...
class TFoodEvaluator(object):
    def __init__(self, config, device) -> None:
        ...
        self.dataloader = create_data(config, 'test')
        logger.info('creating models')
        self.model = create_tfood_model(config,device)
        vlp_path = config.VLP_PATH
        org_state_dict = torch.load(vlp_path,map_location=device)
        new_state_dict = convert_state_dict(org_state_dict)
        self.model.load_state_dict(new_state_dict)
...
class TFoodClipEvaluator(object):
    def __init__(self, config, device) -> None:
        self.config = config
        self.device = device
        logger.info('creating data loader')
        self.dataloader = create_data(config, 'test')
        logger.info('creating models')
        self.model = create_tfood_clip_model(config,device)
        vlp_path = config.VLP_PATH
        org_state_dict = torch.load(vlp_path,map_location=device)
        new_state_dict = convert_tfood_clip_state_dict(org_state_dict)
        self.model.load_state_dict(new_state_dict)
        self.loss_function = TripletLoss()
...
class NutrTFoodAllEvaluator(object):
    def __init__(self, config, device) -> None:
        self.config = config
        self.device = device
        logger.info('creating data loader')
        self.dataloader = create_data(config, 'test')
        logger.info('creating models')
        self.model = create_nutr_tfood_model(config,device)
        self.model.load_state_dict(torch.load(os.path.join(config.SAVE_PATH,config.RECIPE_MODEL_PATH),map_location=device))
        self.loss_function = TripletLoss()
...
class NutrTFoodDirectEvaluator(object):
    def __init__(self, config, device) -> None:
        self.config = config
        self.device = device
        logger.info('creating data loader')
...
class NutrTFoodDirectIngrsEvaluator(object):
    def __init__(self, config, device) -> None:
        self.config = config
        self.device = device
        logger.info('creating data loader')
...
class NutrCarEvaluator(object):
    def __init__(self, config, device) -> None:
        self.config = config
        self.device = device
        logger.info('creating data loaders')
        self.dataloader = create_recipe_retrieval_data(config, 'test')
        self.loss_function = TripletLoss()
        self.recipe_loss_function = MultiTripletLoss()
        logger.info('creating models')
        self.model = create_nutr_car_model(config,device)
        self.model.load_state_dict(torch.load(os.path.join(config.SAVE_PATH,config.RECIPE_MODEL_PATH),map_location=device))
    ...
    def evaluate(self):
        ...
        for batch in tqdm(self.dataloader):
            ...
            evaluator(out['embs']['img'], out['embs']['recipe'],batch)
            for id, pred_n in zip(ids, pred_nutr):
                out_dict[id] = {name: pn.item()*nutr_scale[name] for name,pn in zip(nutr_names,pred_n)}
            pred_ingr_indexes = [torch.nonzero(pred_ingr.bool(),as_tuple=True)[0].tolist() for pred_ingr in F.sigmoid(pred_ingrs) > 0.2]
            for id, pred_i in zip(ids,pred_ingr_indexes):
...
class NutrOnlyHTEvaluator(object):
    def __init__(self, config, device) -> None:
        self.config = config
        self.device = device
        logger.info('creating data loader')
...
class NutrIngrOnlyHTEvaluator(object):
    def __init__(self, config, device) -> None:
        self.config = config
        self.device = device
        logger.info('creating data loader')
