...
def evaluate(config, dataloader: DataLoader, recipe_model: nn.Module, img_model: nn.Module, nutr_model: nn.Module, im2rec_loss_function: nn.Module, recipe_loss_function: nn.Module, device):
    ...
    for batch in tqdm(dataloader):
        ...
        _, out_comp_embs = recipe_model(recipe_input,nutr=nutr)
        out_img = img_model(img_input)
        out_nutr = nutr_model(nutr)
        loss = im2rec_loss_function(out_img,out_nutr) + recipe_loss_weight * recipe_loss_function(out_comp_embs)
...
def main():
    ...
    nutr_model.load_state_dict(torch.load(nutr_model_path,map_location=device))
    loss_function = TripletLoss()
    recipe_loss_function = MultiTripletLoss()

    evaluate(config, dataloader, recipe_model, img_model, nutr_model, loss_function, recipe_loss_function, device)
