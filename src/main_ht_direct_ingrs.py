...
def train_one_epoch(config, dataloaders: Dict[str, DataLoader], model: nn.Module, im2rec_loss_function: nn.Module, optim, device):
    ...
    for phase in ['train','val']:
        ...
        for batch in tqdm(dataloaders[phase]):
            ...
            retrieval_loss = im2rec_loss_function(out_img,out_text,nutrs=nutr)
            mae_loss = config.TRAIN.MAE_WEIGHT * cal_mae(nutr,pred_nutr)
...
def train(config, dataloaders: Dict[str, DataLoader], model: nn.Module, im2rec_loss_function: nn.Module, optim1, optim2, scheduler, device):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch < warmup_epochs:
            avg_loss_dict = train_one_epoch(config, dataloaders, model, im2rec_loss_function, optim1, device)
        else:
            avg_loss_dict = train_one_epoch(config, dataloaders, model, im2rec_loss_function, optim2, device)
...
def main():
    ...
    if not config.TRAIN.FINETUNE:
        # model.joint_embedding.image_encoder.requires_grad_(False)
        model.joint_embedding.text_encoder.requires_grad_(False)
    elif config.TRAIN.FINETUNE_MODEL == 'clip':
        model.joint_embedding.text_encoder.requires_grad_(False)
    loss_function, _ = get_loss(config)
    if not config.TRAIN.FINETUNE:
        params = list(model.joint_embedding.image_encoder.fc.parameters()) + \
            list(model.joint_embedding.merger_recipe.parameters()) + \
            list(model.nutr_decoder.parameters()) + \
            list(model.ingr_decoder.parameters()) + \
            list(model.nutr_embedder.parameters())
    elif config.TRAIN.FINETUNE_MODEL == 'clip':
        params = list(model.joint_embedding.merger_recipe.parameters()) + \
            list(model.nutr_embedder.parameters()) +\
            list(model.nutr_decoder.parameters()) + \
            list(model.ingr_decoder.parameters()) +\
            list(model.joint_embedding.image_encoder.parameters())
        ...
        params = list(model.joint_embedding.image_encoder.fc.parameters()) + \
            ...
            list(model.nutr_decoder.parameters()) + \
            list(model.ingr_decoder.parameters()) +\
            list(model.joint_embedding.text_encoder.parameters())
    else:
        params = list(model.parameters())
    warmup_params = list(model.nutr_embedder.parameters()) \
        + list(model.nutr_decoder.parameters()) \
        + list(model.ingr_decoder.parameters())
    ...
    train(config, dataloaders, model, loss_function, optim1, optim2, scheduler, device)
