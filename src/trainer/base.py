...
# from model.nutr_car import create_nutr_car_model
...
# from model.car import create_car_model
...
class BaseTFoodTrainer(ABC):
    def __init__(self, config, device) -> None:
        ...
        org_state_dict = torch.load(vlp_path,map_location=device)
        new_state_dict = convert_state_dict(org_state_dict)
        self.model.tfood.load_state_dict(new_state_dict)
        self.loss_function, self.recipe_loss_function = get_loss(config)
...
class BaseCarTrainer(ABC):
    def __init__(self, config, device) -> None:
        self.config = config
        self.device = device
        phases = ['train', 'val']
        logger.info('creating data loaders')
        self.dataloaders = {phase: create_recipe_retrieval_data(config, phase) for phase in phases}
        self.loss_function, self.recipe_loss_function = get_loss(config)
        logger.info('creating models')
        self.model = create_car_model(config,device)
        print_trainable_parameters(self.model)
        if config.TRAIN.RESUME:
            print('load checkpoint')
            self.model.load_state_dict(torch.load(config.TRAIN.CHECKPOINT))
        if config.TRAIN.FINETUNE:
            print('load model for finetuning')
            self.model.load_state_dict(torch.load(config.TRAIN.CHECKPOINT))
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            logger.info(f'epoch {epoch + 1}')
            avg_loss_dict = self.train_one_epoch(self.optim)
            self.scheduler.step(epoch+1)
            # val_loss = avg_loss_dict['val']
            # if val_loss < min_val_loss:
            #     min_val_loss = val_loss
...
class BaseNutrCarTrainer(ABC):
    def __init__(self, config, device) -> None:
        ...
        if config.TRAIN.FINETUNE:
            print('load model for finetuning')
            self.model.car.load_state_dict(torch.load(config.TRAIN.CHECKPOINT))
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            if val_loss < min_val_loss:
                ...
                best_epoch = epoch + 1
            if (epoch) % 10 == 0:
                torch.save(best_model.state_dict(),os.path.join(self.config.SAVE_PATH,'checkpoints',f'food_ep{epoch+1}.pt'))
        logger.info(f'best epoch: {best_epoch}')
        logger.info(f'min val loss: {min_val_loss}')
        torch.save(best_model.state_dict(),os.path.join(self.config.SAVE_PATH,'food_test.pt'))
