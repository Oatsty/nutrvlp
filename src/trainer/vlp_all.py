...
class VLPTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
        super(VLPTrainer, self).__init__(config, device)
        if not config.TRAIN.FINETUNE:
            self.model.vlp_cook.recipe_embedder.requires_grad_(False)
            self.model.vlp_cook.image_embedder.requires_grad_(False)
        if not config.TRAIN.FINETUNE:
            params = list(self.model.vlp_cook.proj_image.parameters()) + \
                list(self.model.vlp_cook.proj_recipe.parameters()) + \
...
class VLPNoNutrTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
...
class VLP3BranchesTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
        super().__init__(config, device)
        if not config.TRAIN.FINETUNE:
            self.model.vlp_cook.recipe_embedder.requires_grad_(False)
...
class VLPDirectTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
        super().__init__(config, device)
        if not config.TRAIN.FINETUNE:
            self.model.vlp_cook.recipe_embedder.requires_grad_(False)
...
class VLPDirect3BranchesTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
        super().__init__(config, device)
        if not config.TRAIN.FINETUNE:
            self.model.vlp_cook.recipe_embedder.requires_grad_(False)
...
class VLPIngrsOnly3BranchesTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
        super().__init__(config, device)
        if not config.TRAIN.FINETUNE:
            self.model.vlp_cook.recipe_embedder.requires_grad_(False)
        ...
        if not config.TRAIN.FINETUNE:
            ...
            params = list(self.model.vlp_cook.proj_image.parameters()) + \
                ...
                list(self.model.nutr_decoder.parameters()) + \
                list(self.model.ingr_decoder.parameters()) +\
                list(self.model.vlp_cook.image_embedder.parameters())
        elif config.TRAIN.FINETUNE_MODEL == 'ht':
            params = list(self.model.vlp_cook.proj_image.parameters()) + \
                list(self.model.vlp_cook.proj_recipe.parameters()) + \
                list(self.model.nutr_embedder.parameters()) +\
                list(self.model.nutr_decoder.parameters()) + \
                list(self.model.ingr_decoder.parameters()) +\
                list(self.model.vlp_cook.recipe_embedder.parameters())
        else:
            params = list(self.model.parameters())
        self.optim = torch.optim.Adam(
            [
...
class VLPDirectIngrsTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
        super().__init__(config, device)
        if not config.TRAIN.FINETUNE:
            self.model.vlp_cook.recipe_embedder.requires_grad_(False)
...
class VLPDirectIngrs3BranchesTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
        super().__init__(config, device)
        if not config.TRAIN.FINETUNE:
            self.model.vlp_cook.recipe_embedder.requires_grad_(False)
...
class VLPDirectIngrsNoNutrTrainer(BaseVLPTrainer):
    def __init__(self, config, device) -> None:
        super().__init__(config, device)
        if not config.TRAIN.FINETUNE:
            self.model.vlp_cook.recipe_embedder.requires_grad_(False)
