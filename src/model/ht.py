...
class HT(nn.Module):
    def __init__(self, output_size, image_model='resnet18', vocab_size=None, hidden_recipe=512, n_heads=4, n_layers=2):
        super(HT, self).__init__()
        self.joint_embedding = JointEmbedding(output_size, image_model, vocab_size, hidden_recipe, n_heads, n_layers)
...
def create_ht_model(config,device):
    ...
    return HT(output_size,image_model,vocab_size,hidden_recipe,num_heads,num_layers).to(device)
